...
unsigned int ParsePeFileExport(IMAGE_DOS_HEADER* pDosH, unsigned int uiFileLowPart, const char* chNameExport, unsigned __int64* pImageBaseNtdll) {
    ...
    unsigned int checked_range_addr = FixRVAThings(v6, DataDirectory->VirtualAddress, uiFileLowPart);
    ...
    *pImageBaseNtdll = v6->OptionalHeader.ImageBase;
    ...
    unsigned int v11 = FixRVAThings(v6, range_Addr[7], uiFileLowPart);
    unsigned int v12 = FixRVAThings(v6, range_Addr[9], uiFileLowPart);
    unsigned int v13 = FixRVAThings(v6, range_Addr[8], uiFileLowPart);
    ...
    for (unsigned int i = 0; i < v17; ++i) {
        ...
        if (v14 != -1) {
            ...
            if ((v8 < virtualAddress || v8 >= size + virtualAddress) && !strcmp((const char*)pDosH + v14, chNameExport))
                return FixRVAThings(v6, v8, uiFileLowPart);
        }
    }
...
PVOID allocate_and_set_memory(char bFlag, SIZE_T szMemRegion) {
    ...
    PVOID pMemory = ExAllocatePoolWithTag(NonPagedPool, szMemRegion, 'HIDE'); 
...
NTSTATUS OpenNtdllAndParsePE(const char* chNameSyscall, const WCHAR* wNameSyscall) {
    ...
    RtlInitUnicodeString(&ucString, wNameSyscall);
    ...
    NTSTATUS ntStatus = ZwCreateFile(&hFile, 0x80000000, &objectAttributes, &ioStatusBlock, 0i64, 0x80u, 1u, 1u, 0x20u, 0i64, 0);
    ...
    if (NT_SUCCESS(ZwQueryInformationFile(hFile, &ioStatusBlock, &FileInformation, 0x18u, FileStandardInformation))) {

        struct_malware.fileLowPart = FileInformation.EndOfFile.LowPart;

        struct_malware.pMemBuffer = allocate_and_set_memory(1, struct_malware.fileLowPart);

        byteOffset.QuadPart = 0i64;

        ZwReadFile(hFile, 0i64, 0i64, 0i64, &ioStatusBlock, struct_malware.pMemBuffer, struct_malware.fileLowPart, &byteOffset, 0i64);
    }
...
uint32_t parserpentdll(const char* nameexport) {

    WCHAR wNtDll[32] = { 0 };

    memcpy(wNtDll, L"\\SystemRoot\\system32\\ntdll.dll", 0x3Eui64);

    uint32_t ssdtExportedIndex = 0;

    if (OpenNtdllAndParsePE(nameexport, wNtDll) < 0)
        return  0xFFFFFFFF;

    char* pChecking = (char*)struct_malware.pMemBuffer + struct_malware.section_range;

    DbgPrintEx(0, 0, "Going seek: %X < %X", struct_malware.section_range, (unsigned __int64)struct_malware.fileLowPart);
    ...
    for (int i = 0;
        ...
        DbgPrintEx(0, 0, "Seeking: %X", pChecking[i]);
    ...
    if (struct_malware.pMemBuffer)
        ZwClose(struct_malware.pMemBuffer);
...
unsigned int FindSyscallIndexOnSsdt(const char* nameexport) {
    ...
    unsigned int p_find_syscall_addy = parserpentdll(nameexport);

    DbgPrintEx(0, 0, "SSDT Syscall INDEX: 0x%X", p_find_syscall_addy);
...
PVOID GetNtOsKrnl(PULONG puImageSize) {
    ...
    ZwQuerySystemInformation(SystemModuleInformation, &ulSystemBufferSize, 0, &ulSystemBufferSize);

    if (!ulSystemBufferSize) return (PVOID)0;

    PSYSTEM_MODULE_INFORMATION  pSystemInfoBuffer = (PSYSTEM_MODULE_INFORMATION)ExAllocatePoolWithTag(NonPagedPool, ulSystemBufferSize * 2, 'xxp');

    if (!pSystemInfoBuffer) return (PVOID)0;

    memset(pSystemInfoBuffer, 0, ulSystemBufferSize * 2);

    ZwQuerySystemInformation(SystemModuleInformation, pSystemInfoBuffer, ulSystemBufferSize * 2, &ulSystemBufferSize);

    PVOID pModuleBase = pSystemInfoBuffer->Module[0].ImageBase;
    
    *puImageSize = pSystemInfoBuffer->Module[0].ImageSize;
    
    DbgPrintEx(0, 0, "Path: %s", pSystemInfoBuffer->Module[0].FullPathName);
...
PSSDTStruct GetFunction() {
    ...
    ULONG_PTR PLACE = (ULONG_PTR)GetNtOsKrnl(&pImageSize);

    PIMAGE_NT_HEADERS ntHeaders = RtlImageNtHeader((PVOID)PLACE);

    PIMAGE_SECTION_HEADER pTextSection = IMAGE_FIRST_SECTION64(ntHeaders);

    ULONG KiSSSOffset;

    for (KiSSSOffset = 0; KiSSSOffset < pTextSection->Misc.VirtualSize - sizeof(KiSystemServiceStartCodePattern); KiSSSOffset++)
        if (RtlCompareMemory(((unsigned char*)PLACE + pTextSection->VirtualAddress + KiSSSOffset), KiSystemServiceStartCodePattern, sizeof(KiSystemServiceStartCodePattern)) == sizeof(KiSystemServiceStartCodePattern)) 
            break;
...
PVOID GetFunctionAddress(const char* apiname) {
    ...
    ULONG readOffset = (ULONG)FindSyscallIndexOnSsdt(apiname);
